...
float ActionneurDriver::calculateMaxCurrent()
{
    ...
    {
        maxHori = param.m_maxAmpUpHori/2;
        maxVert = param.m_maxAmpUpVert/2;
        DEBUGLOGF("WARNUNG MAX AMP DEFAULT status [%d] -intensity [hori %f, vert %f]\n", m_status, maxHori, maxVert);
    }
    ...
    case ACTIONEUR_TRAPPE_AND_HORIZONTAL:
        ...
        break;
    default:
        m_maxCurrentAccepted = param.m_maxPowerAmp;
        DEBUGLOGF("WARNUNG MAX AMP DEFAULT m_actioneur [%d] -intensity Max[%f]\n", m_actioneur,param.m_maxPowerAmp);
    }

    return m_maxCurrentAccepted;
}

void ActionneurDriver::handle(float fIntensity)
{
    ...
    float maxAmpCalculated=calculateMaxCurrent();
    if (fIntensity > maxAmpCalculated /*param.m_maxPowerAmp*/ && m_delayIntensity.isDone())
    {
        DEBUGLOGF("Over intensity [%f/ max %f] ==> STOP\n", fIntensity, maxAmpCalculated /*param.m_maxPowerAmp*/);
        stopActionneur();
        return;
    }
        ...
        m_delayIntensity.isDone())
    {
        if (m_actioneur == ACTIONEUR_HORIZONTAL)
        {
            ...
            DEBUGLOGF("under intensity while opening -intensity [%f/ min %f] ==> CYCLE HORIZONTAL FINISHED \n", fIntensity, param.m_minPowerAmp);
            intensityCtl.clean();
        }
        else
        {
            DEBUGLOGF("under intensity while closing or opening -intensity [%f/ min %f] ==> CYCLE CLOSING/OPENING FINISHED \n", fIntensity, param.m_minPowerAmp);
